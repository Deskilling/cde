...
# -trimpath replaces all filesystem paths with relative paths
GOFLAGS := -trimpath
# -s disables symbol table
# -w disable DWARF generation
LDFLAGS := -ldflags="-s -w"

# Cgo allows Go code to call C functions and vice versa, but produces a dynamically linked binary.
# Disabling it forces a statically linked binary, which is recommended for cross-platform builds.
CGO_ENABLED := 0
